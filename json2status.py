...
def main():
    ...
    parser.add_option("-s", "--status", dest="status_file", default="infra_status.json",
    ...
    process(options.stations_file, options.path, options.outdir, options.status_file)

def process(file, path, outdir, output_file_name):
    import string
    import re
    import os
    import pydash
        ...
        with open(json_dir, "r") as infile:
           ...
           # print(group_name)
            ...
            if os.path.exists(output_path):
                with open(output_path, "r") as jsfile:
                    js_object = json.load(jsfile)
                    js_sta = js_station.lower()
                    """
                    for key in js_object:
                        #print(key)
                        if key == group_name:
                            #print(js_object[key])
                            for key2 in js_object[key]:
                                print(js_object[key][key2]['pos'])
                                #for key3 in js_object[key][key2]:
                                #    print(key3)
                    """
                   # print(group_name, js_sta)

#                    print(js_object[group_name], len(js_object[group_name]))
                    if len(js_object[group_name]) != 0 and  js_pos:
                        js_pos = js_object[group_name][js_sta]['pos']
                    #print(js_pos)
                        if js_pos == "Fixed":
                            lat = js_object[group_name][js_sta]['lat']
                            lon = js_object[group_name][js_sta]['lon']
                        jsfile.close()
            else:
                js_pos = "Unknown"
            ...
            if js_pos == "Fixed":
                new_sta_json['pos'] = js_pos
            else:
                new_sta_json['pos'] = json_object['Position']

            #new_sta_json['fixed'] = 1
            #print(new_sta_json)
            ...
            new_group_json[group_name]['display'] = 1
            new_group_json[group_name][js_station] = new_sta_json
            #new_group_json[group_name]['reg'] = new_sta_json
    ...
    #print(json_object)
    #output_file_name = "infra_status.json"
