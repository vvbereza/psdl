...
#            print(len(index_line))
        ...
        if os.path.exists(json_dir):
            ...
            with open(json_dir, "r") as infile:
                ...
                prev_gps_status_json[station] = ""
                if len(json_object) == 0:
                    gps_status_json[station] = ""
                else:
#                    print("json not empty")
                    if 'Position' in json_object:
                        prev_gps_status_json[station] = json_object["Position"]
                        gps_status_json[station] = prev_gps_status_json[station]
                    else:
                        gps_status_json[station] = ""
                        prev_gps_status_json[station] = ""

                    if 'Coords' in json_object:
                        prev_coords_json[station] = json_object['Coords']
                    else:
                        gps_status_json[station] = ""
                        prev_coords_json[station] = ""

                    if 'Nav. status' in json_object:
                        prev_navstat_json[station] = json_object['Nav. status']
                    else:
                        prev_navstat_json[station] = "0"

                    prev_navstat_splitted = prev_navstat_json[station].split()
                    if len(prev_navstat_splitted) > 1:
                        prev_navstat_level_items = prev_navstat_splitted[1]
                        prev_navstat_level[station] = prev_navstat_level_items[1:2]
                    else:
                        prev_navstat_level[station] = "0"
               ...
               # print(lon_splitted)
                    if lon_splitted and (lon_splitted[0] == '0' or lon_splitted[0] == 'N/A'):
                        prev_lon = lon_splitted[0]
                    # print(prev_lon)
                    elif lon_splitted:
                    ...
                    if len(lon_splitted) > 0:
                        prev_lon_json[station] = lon_splitted[0]
                    else:
                        prev_lon_json[station] = "0"
                    #print(prev_lon_json[station])
                    #print(prev_lon_json[station])
                    ...
                    if lat_splitted and (lat_splitted[0] == '0' or lat_splitted[0] == 'N/A'):
                        lat = lat_splitted[0]
                    # print(lat_splitted)
                    elif lat_splitted:
                    ...
                    if len(lat_splitted) > 0:
                        prev_lat_json[station] = lat_splitted[0]
                    else:
                        prev_lat_json[station] = "0"
                    #print(prev_lat_json[station])
                #print(prev_lat_json[station])
               # print(json_object['Position'])
                    if 'Position' in json_object:
                        prev_gps_status_json[station] = json_object['Position']
                        gps_status_json[station] = prev_gps_status_json[station]
                        #print(prev_gps_status_json)
                    else:
                        prev_gps_status_json[station] = "Flow"
                        gps_status_json[station] = prev_gps_status_json[station]
                    #print("index_file_exist[%s] = %s" % (station, index_file_exist[station]))
                    if index_file_exist[station] == False:
                        ...
                        #print("index_file_exist(True) = %s" % index_file_exist[station])
                        station_json['Status'] = 'offline'
                        status[station] = "offline"
            ...
            if index_file_exist[station] == False:
                ...
                if prev_gps_status_json != "Fixed":
                    station_json['Coords'] = ""
                    station_json['Latitude'] = ""
                    station_json['Longitude'] = ""
                    gps_status_json[station] = "Flow"
                    station_json['Position'] = "Flow"
                elif prev_gps_status_json == "Fixed":
                    station_json['Position'] = "Fixed"
                    gps_status_json[station] = "Fixed"
                else:
...
#                print(station_json['Power'])
            ...
            if index >= 0:
                coords_item = index_line[index + 15:index + 45]
                coords_splitted = coords_item.rsplit('<')
                coords = coords_splitted[0]
                station_json['Coords'] = coords
                #print(coords)
                lat_lon = coords.split(", ")
                if lat_lon:
                    station_json['Latitude'] = lat_lon[0]
                    lat_splitted = lat_lon[0].split()
                    # print(lon_splitted)
                    ...
                    station_json['Longitude'] = lat_lon[1]
#                    station_json['Latitude'] = lat

                    lon_splitted = lat_lon[1].split()
                    # print(lat_splitted[0])
...
#                    station_json['Longitude'] = lon
            # lat += 0.00001
               # lon += 1
                if (prev_navstat_level and prev_navstat_level[station] and prev_navstat_level[station] == "1" \
                        and len(prev_coords_json[station]) > 0) or prev_gps_status_json[station] == "Fixed" :
                   station_json['Coords'] = prev_coords_json[station]
                   station_json['Latitude'] = prev_lat_json[station]
                   station_json['Longitude'] = prev_lon_json[station]
                   station_json['Position'] = "Fixed"
                   gps_status_json[station] = "Fixed"
                   #station_json[station] = gps_status_json[station]
                   #print(station_json['Coords'])
                   #print(station_json['Latitude'])
                   #print(station_json['Longitude'])
                elif prev_gps_status_json[station] != "Fixed" and gps_status_json[station] != "Fixed":
                    gps_status_json[station] = "Flow"
                    station_json['Position'] = gps_status_json[station]
                    #station_json[station] = gps_status_json[station]
                    #print(station, gps_status_json[station])
#                    return

                """
                if lat != 0.0 and lon != 0.0 and lat != 'N/A' and lon != 'N/A' and prev_lat_json and prev_lon_json:
                    lat_diff = abs(lat - prev_lat_json[station])
                    lon_diff = abs(lon - prev_lon_json[station])
                #print(prev_lat_json[station], prev_lon_json[station])
                #print(lat, lon)
                    if lat_diff <= 0.000009 and lon_diff <= 0.000009:
                        gps_status_json[station] = "Fixed"
                        station_json['Position'] = gps_status_json[station]
                    else:
                        gps_status_json[station] = "Flow"
                    ...
                    gps_status_json[station] = "Unknown"
                    station_json['Position'] = gps_status_json[station]
                """
...
#                    nav_status_level_items = 'X'
                ...
                if nav_status_level == "1" or prev_gps_status_json[station] == "Fixed":
                    gps_status_json[station] = "Fixed"
                    station_json['Position'] = gps_status_json[station]
                elif gps_status_json[station] == 'Fixed':
                    station_json['Position'] = gps_status_json[station]
                else:
                    gps_status_json[station] = "Flow"
                    station_json['Position'] = gps_status_json[station]
